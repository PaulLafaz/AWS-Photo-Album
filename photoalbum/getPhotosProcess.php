...
<body>
<h1>Photo Searching Result Page</h3>
<?php
/*
...
*/
	...
	$host = "assignmet1b-db.cun0793gtz8l.us-east-1.rds.amazonaws.com";
	$user = "admin"; 
			...
			if($keywordsExists == true)
			{
				if($keywordsExists == true)			//If all 3 of the field where filled out then
				{
					...
					while ($row = mysqli_fetch_row($keywordsResult))		
					{
						for($i = 0; $i < count($keyword); $i++)
						{
							if($row[2] == $keyword[$i])					//Store all the keywords from the database that are related
